...
export default function NavBar() {
	...
	useEffect(() => {
		const handleScroll = () => {
			...
			if (horizontalNavbar) {
				if (window.scrollY > 10) {
					horizontalNavbar.classList.add('py-3');
					horizontalNavbar.classList.remove('py-8');
				} else {
					horizontalNavbar.classList.add('py-8');
					horizontalNavbar.classList.remove('py-3');
				}
			}
		};
	...
	return (
		<>
			<nav
				className="sticky top-0 z-10 w-full border-b-2 px-8 py-8 text-white transition-all ease-in-out dark:text-white md:fixed"
